...
pub mod winver_tests {
    ...
    fn test_ver_query_value() {
        ...
        let vfi_a =
            ver_query_value_a(&fvi_a, VerQuerySubBlock::VarFileInfo).unwrap();
        let vfi_w =
            ver_query_value_w(&fvi_w, VerQuerySubBlock::VarFileInfo).unwrap();
        ...
        .unwrap();
        assert_eq!(
            sfi_a.as_string_file_info().to_string_lossy(),
            sfi_w.as_string_file_info().to_string_lossy()
        )
    }
}
